...
// html+javascript혼합 처리(권장=리액트)
	...
	function deleteForm(){
		// 리액트 계속 - import(ES6, module, ES6 기준문법-안전함, require-CommonJS)
		$("#dlg_boardDel").dialog({
			title: '글삭제 - 비밀번호 확인',
	...
	function boardDel(){
		console.log("boardDel 호출성공");
		// 자바스크립트에서는 값에 ''이나 ""을 안붙이면 변수로 처리됨
		let db_pw = "<%=rb_pw%>";
...
<!-- 글 삭제 시작 -->	
	<div id="dlg_boardDel" footer="#btn_boardDel" class="easyui-dialog" title="삭제하기" data-options="modal:true,closed:true" style="width:300px;height:200px;padding:10px">
...
<!-- 수정 화면 추가 시작 -->
    <div id="dlg_boardUpd" footer="#tb_boardUpd" class="easyui-dialog" title="수정하기" data-options="modal:true,closed:true" style="width:600px;height:400px;padding:10px">
        <!-- 바이너리(문자열+숫자) 파일을 전송할 때는 반드시 post방식 처리 첨부파일 처리 시 -->
...
<!-- 		바이너리 (문자열+숫자) 파일을 전송할 때는 반드시 post 방식으로 처리한다 -->
<!-- 		base2, base8, base16, base64 텍스트파일 (문자열 치환-파일 크기가 커진다) -->
<!--        <form id="f_boardIns" method="post" enctype="multipart/form-data" action="./boardInsert.sp"> -->
        <form id="f_boardIns" method="get" action="./boardInsert.sp">
        <!-- 태그 안에 자바 변수 사용 가능한가? > 가능함 -->
